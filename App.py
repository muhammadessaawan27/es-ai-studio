...
# ==========================================
# 1. INDUSTRIAL STABILITY & RULE 61 CORE
# ==========================================
...
# ==========================================
# 2. WHITE-LABEL UI & BRANDING (RULE 64.5)
# ==========================================
st.set_page_config(page_title="Sglowina AI - Official Enterprise Titan", layout="wide", page_icon="🎬")
...
# ==========================================
# 3. IDENTITY FIREWALL (LOCKED OFFICIAL BIO)
# ==========================================
SGL_BIO = """
Sglowina AI is proudly developed by the Sglowina Team.

**ES FOUNDERS & CEOs:** Muhammad Essa Awan & Saba Wahid.

Saba Wahid is the Founder & CEO of Sglowina AI, daughter of Wahid Bakhsh. 
Muhammad Essa Awan is the Founder & CEO, professional Mechanical Engineer, and the lead logical architect.
...
def create_v40_titan_movie(story, voice, ratio, style, seed):
    ...
    try:
        ...
        res_map = {"YouTube (16:9)": (1280, 720), "TikTok/Reels (9:16)": (720, 1280), "Instagram (1:1)": (1024, 1024), "Ultra-Wide (21:9)": (2560, 1080)}
        w, h = res_map[ratio]
...
if not st.session_state.user:
    ...
    # Branded Header ( Muhammad First only in Bio, ES here )
    st.markdown('<div class="executive-header"><div style="text-align:center; font-family:\'Orbitron\'; font-weight:900; color:#fff; letter-spacing:3px;">ES FOUNDER & CEOs | SGLOWINA AI</div></div>', unsafe_allow_html=True)
    ...
    if menu == "🎨 Image Studio":
        ...
        with c2: i_r = st.selectbox("Size:", ["Square (1:1)", "YouTube HD (16:9)", "TikTok (9:16)", "Ultra-Wide (21:9)"])
        ...
        with cv1: v = st.selectbox("Narrator:", ["Male", "Female"])
        with cv2: r = st.selectbox("Format:", ["YouTube (16:9)", "TikTok (9:16)", "Ultra-Wide (21:9)"])
        ...
        if st.button("Render Official Movie"):
            res = create_v40_titan_movie(m_script, v, r, s, u['id'], seed)
        ...
        if p := st.chat_input("Hukum..."):
            ...
            else: res = requests.get(f"https://text.pollinations.ai/{urllib.parse.quote(p)}?model=openai&cache=true").text.replace("ChatGPT", "Sglowina AI")
