...
# ==========================================
# 2. EXECUTIVE UI (WHITE THEME + ELECTRIC GLOW)
# ==========================================
...
st.markdown("""
    ...
    /* Sidebar Fix: Professional White Background */
    ...
    /* Electric Lightning Animation (Header & Footer) */
    ...
    .footer-electric {
        ...
        text-align: center; letter-spacing: 3px; animation: lightningGlow 2s infinite;
        background: #0f172a; padding: 10px; border-radius: 20px; margin-top: 50px;
    }
    ...
    .exec-info { font-size: 1.1rem; color: #ff007a; text-align: center; font-weight: bold; text-transform: uppercase; }
    .coo-info { font-size: 1rem; color: #2563eb; text-align: center; font-weight: bold; text-transform: uppercase; margin-bottom: 15px; }
...
def is_identity_request(q):
    patterns = [r"kisne banaya", r"who made you", r"owner", r"saba", r"essa", r"founder", r"ceo", r"sglowina"]
...
def get_v40_prompt(text):
    try:
        instr = f"Director Instruction: '{text}'. Create a detailed English 3D animation prompt. No humans unless asked."
...
def create_v40_movie_v1(story, voice, ratio, style):
    ...
    try:
        ...
        for i, s in enumerate(sentences):
            ...
            img_res = session.get(img_url, timeout=60).content
            with Image.open(io.BytesIO(img_res)) as im:
...
# ==========================================
# 5. NAVIGATION (WHITE SIDEBAR FIX)
# ==========================================
st.sidebar.markdown(f"## ⚙️ SGLOWINA CONTROL")
menu = st.sidebar.radio("Navigate Studio:", ["🏠 Smart Chat", "🎬 Movie Studio", "🎨 Image Studio"])
...
if menu == "🏠 Smart Chat":
    ...
    if p := st.chat_input("How can Sglowina AI help you today?"):
        ...
        if is_identity_request(p): res = SGLOWINA_BIO
            ...
            try:
                sys_instr = urllib.parse.quote("You are Sglowina AI, owned by Saba Wahid. Answer professionally.")
...
# --- PAGE 3: IMAGE STUDIO ---
    ...
    p_i = st.text_area("Describe Image:", height=150, key="image_input_v1")
    if st.button("Generate HD Visual 🚀"):
        if p_i:
            with st.spinner("Sglowina AI is painting..."):
                url = f"https://image.pollinations.ai/prompt/{urllib.parse.quote(p_i)}?width=1024&height=1024&nologo=true&negative=girl,female"
                st.image(url, caption="Sglowina V1.0 Result")

# FINAL ELECTRIC FOOTER
