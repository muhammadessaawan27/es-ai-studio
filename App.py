...
st.markdown("""
    ...
    .stTextArea>div>div>textarea, .stTextInput>div>div>input {
        background-color: #ffffff !important; border: 1px solid #cbd5e1 !important; border-radius: 12px !important; color: #000000 !important;
    }
    </style>
...
# ==========================================
# 4. v40 TITAN MOVIE ENGINE (CONSISTENCY FIXED)
# ==========================================
def get_v40_prompt(text, style, seed):
    try:
        instr = f"Director Order: '{text}'. Professional 3D character animation. Ensure the face and clothing match the reference seed {seed}. No humans unless asked."
...
def create_titan_movie_v1(story, voice, ratio, style, part, seed):
    ...
    try:
        ...
        # FIXED: Every scene uses the SAME seed for character consistency
        img_urls = [f"https://image.pollinations.ai/prompt/{urllib.parse.quote(get_v40_prompt(s, style, seed))}?width={w}&height={h}&seed={seed}&nologo=true&negative=girl,female,deformed" for s in sentences]
...
# ==========================================
# 5. NAVIGATION & FULL MODULES
# ==========================================
st.sidebar.markdown(f"## ⚙️ SGLOWINA MENU")
page = st.sidebar.radio("Navigate Studio:", ["🏠 Smart Chat", "🎬 Movie Studio (Parts)", "🎨 Pro Image Studio (Full)"])
...
# --- PAGE 2: MOVIE STUDIO ---
    ...
    char_id_movie = st.number_input("Character Identity Lock (Seed):", value=st.session_state.char_seed)
    
    m_script = st.text_area("Enter Story Script:", height=150)
    ...
    if st.button("Generate Master Movie Part 🚀"):
        if m_script:
            v_res = create_titan_movie_v1(m_script, mv, mr, ms, p_num, char_id_movie)
            if "mp4" in v_res: st.video(v_res); st.download_button("Download", open(v_res, 'rb').read(), file_name=v_res)

# --- PAGE 3: IMAGE STUDIO ---
elif page == "🎨 Pro Image Studio (Full)":
    ...
    with ic2: i_size = st.selectbox("Resolution:", list(ratio_opts.keys()))
    with ic3: is_count = st.slider("Quantity per Prompt:", 1, 10, 1)
    
    char_id = st.number_input("Character ID (Lock face):", value=st.session_state.char_seed)

    if st.button("Generate Titan Visuals 🚀"):
        w, h = ratio_opts[i_size]
        ...
        for idx, single_p in enumerate(prompt_list):
            for q in range(is_count):
                with st.spinner(f"Painting image {idx+1}..."):
                    ...
                    st.image(url, caption=f"ID: {char_id}")
