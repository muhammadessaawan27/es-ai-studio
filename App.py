...
st.markdown("""
    ...
    .stTextArea>div>div>textarea, .stTextInput>div>div>input {
        background-color: #ffffff !important; border: 2px solid #cbd5e1 !important; border-radius: 10px !important; color: #000000 !important;
    }
    </style>
...
page = st.sidebar.radio("Go To:", ["🏠 Smart Chat", "🎥 Movie Studio (Parts)", "🎨 Pro Image Studio (Full)"])
...
# --- PAGE 2: MOVIE STUDIO ---
    ...
    st.write("### 🎥 Industrial Cinematic Production (v40 Logic)")
...
# --- PAGE 3: IMAGE STUDIO (FIXED ALL FIELDS) ---
elif page == "🎨 Pro Image Studio (Full)":
    st.write("### 🎨 Industrial Visual Studio")
    p_i = st.text_area("Describe Image (One per line for batch):", height=150, key="p_i_area")
    ...
    with ic2: i_size = st.selectbox("Resolution:", list(ratio_opts.keys()), key="ir_box")
    with ic3: is_count = st.slider("Quantity per Prompt:", 1, 10, 1, key="ic_slider")
    
    char_id = st.text_input("Character ID (Seed) for Consistency:", value=str(st.session_state.char_seed), key="char_seed_box")

    if st.button("Generate HD Industrial Visuals 🚀"):
        if p_i:
        ...
        else: st.warning("Please enter a description.")
