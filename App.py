...
# ==========================================
# 2. PREMIUM MINIMAL UI (WHITE & ELECTRIC)
# ==========================================
...
def get_v40_prompt(text):
    try:
        instr = f"Director: '{text}'. Professional 3D character animation. High detail anatomy. Output ONLY English prompt."
...
if menu == "🏠 Smart Chat":
    ...
    if p := st.chat_input("How can Sglowina Titan help you?"):
        ...
        with st.spinner("Sglowina AI is searching..."):
    ...
    st.write("### 🎥 Modular Cinematic Studio (10 min support)")
    part_n = st.number_input("Part Number:", min_value=1, step=1, value=1)
    if st.button("Start New Story (Reset Character)"):
        st.session_state.char_seed = random.randint(1, 999999); st.success("Character Identity Locked!")
    m_s = st.text_area("Enter Script:", height=150)
    ...
    if st.button("Generate Master Movie 🚀"):
        v_res = create_titan_movie_v1(m_s, mv, mr, ms, part_n)
