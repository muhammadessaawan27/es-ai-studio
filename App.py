...
# ==========================================
# 2. ESSA IDENTITY & v40 ENGINE (RESTORED)
# ==========================================
ESSA_BIO = """
...
def get_v40_visual_prompt(urdu_text, style):
    try:
        instr = f"Act as a Film Director. Extract only the primary visual subject from Urdu: '{urdu_text}'. Describe it in detail in English for a 3D animation. Accurate animals/objects. No humans unless mentioned. Output ONLY the English prompt."
        ...
        return f"{style} cinematic style, {desc}, high detail, 8k, vibrant lighting"
...
def create_v40_movie_engine(story, voice_gen, ratio, style):
    ...
    try:
        ...
        # v40 SENTENCE SPLITTING LOGIC (Fixed to ensure image change)
...
# ==========================================
# 3. UNIVERSAL IMAGE STUDIO (v45 Precision)
# ==========================================
def universal_director_ai(user_request, style):
    try:
        instr = f"AI Artist Director: User wants: '{user_request}'. Describe a detailed English AI Image Prompt. Cover Subject, Clothing, Background. If edit mode, KEEP ORIGINAL GENDER. Output ONLY English."
        res = session.get(f"https://text.pollinations.ai/{urllib.parse.quote(instr)}?model=openai", timeout=25)
        desc = res.text if res.status_code == 200 else user_request
        return f"{style} style masterpiece, {desc}, 8k, detailed"
...
def image_studio_pro():
    ...
    if mode == "Text to Image":
        p = st.text_area("تصویر بیان کریں (اردو یا انگلش):")
        ...
        if st.button("Generate Image 🚀"):
            ...
            w, h = res_dim[size]
            refined = universal_director_ai(p, style)
            for i in range(num):
        ...
        if f:
            ...
            edit_p = st.text_area("کیا تبدیلی کرنی ہے؟")
            if st.button("Apply AI Edit 🚀"):
                refined_edit = universal_director_ai(edit_p, "Realistic")
                edit_url = f"https://image.pollinations.ai/prompt/{urllib.parse.quote(refined_edit)}?width=1024&height=1024&nologo=true&negative=girl,female"
...
with t_movie:
    st.write("### 🎥 v40 Proven Movie Engine")
...
st.markdown("---")
st.markdown("<p style='text-align: center; color: #2563eb; font-weight: bold;'>ES AI Studio v50.0 | v40 Engine Restored | Muhammad Essa Awan</p>", unsafe_allow_html=True)
